...
void botonStart(){
	...
	if(digitalRead(PLAY)==1 && digitalRead(PIN_ENT)== 0 && ((flags & FLAG_PLAYER_STOP) || (flags & FLAG_PAUSE))){
...
void botonStop(){
	...
	if((digitalRead(STOP) == 1) && !(flags & FLAG_PLAYER_STOP) && !(flags & FLAG_PLAYER_END) && (flags & FLAG_PAUSE) && digitalRead(PIN_ENT)== 0){
