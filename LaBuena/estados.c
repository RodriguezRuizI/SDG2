...
fsm_trans_t transition_table[] = {
		...
		{WAIT_NEXT, compruebaBotonPausa, WAIT_PAUSE,pausaCancion },
		{WAIT_PAUSE,compruebaPlayerStop,WAIT_START,stopPlayer},
		{WAIT_PAUSE,compruebaBotonStart,WAIT_NEXT, continuaCancion},
		{-1, NULL, -1, NULL }
};
...
void comienzaNuevaNota(fsm_t* this){
	...
	new= (TipoPlayer*) (this->user_data);
	softToneWrite(GPIO_PIN, new->frecuencia_nota_actual);
	lastdur=new->duracion_nota_actual;
	lastfrec=new->frecuencia_nota_actual;
	lastnote=new->posicion_nota_actual;
	free(new->myTimer);
	new->myTimer = tmr_new(callback);
	tmr_startms(new->myTimer,new->duracion_nota_actual);
...
int compruebaBotonStart(fsm_t* this){
	int result = 0;
	piLock (FLAGS_KEY);
	result = (flags & FLAG_PLAYER_START);
	piUnlock (FLAGS_KEY);
...
void continuaCancion(fsm_t* this){
	TipoPlayer* new;
	new= (TipoPlayer*) (this->user_data);
	if(digitalRead(PIN_ENT)== 1){
		flags |= FLAG_PLAYER_STOP;
	}
		new->posicion_nota_actual = lastnote;
		new->frecuencia_nota_actual = lastfrec;
		new->duracion_nota_actual = lastdur;
		softToneWrite(GPIO_PIN, new->frecuencia_nota_actual);
		free(new->myTimer);
		new->myTimer = tmr_new(callback);
		tmr_startms(new->myTimer, new->duracion_nota_actual);
