...
void botonStart(){
	if(millis()<debounceTime2) {
		debounceTime2 = millis() + DEBOUNCE_TIME_PUL ;
		return;
	    }
	if(digitalRead(PLAY)==1 && digitalRead(PIN_ENT)== 0 ){
		piLock(FLAGS_KEY);
...
void botonStop(){
	if(millis()<debounceTime3) {
		debounceTime3 = millis() + DEBOUNCE_TIME_PUL ;
		return;
	    }
	if((digitalRead(STOP) == 1) && digitalRead(PIN_ENT)== 0){
		piLock(FLAGS_KEY);
		flags |= FLAG_PLAYER_STOP;
		flags &= ~FLAG_PLAYER_START;
		piUnlock(FLAGS_KEY);
...
void botonPause(){
	if(millis()<debounceTime4) {
		debounceTime4 = millis() + DEBOUNCE_TIME_PUL ;
		return;
	    }
if(digitalRead(PAUSE) == 1 && digitalRead(PIN_ENT)== 0  && !(flags & FLAG_PLAYER_STOP) && !(flags & FLAG_PLAYER_END)){
		piLock(FLAGS_KEY);
		flags |= FLAG_PAUSE;
		flags &= ~FLAG_PLAYER_START;
		piUnlock(FLAGS_KEY);
...
int systemSetup (void) {
	...
	pullUpDnControl (PLAY, PUD_DOWN) ;
	pullUpDnControl (STOP, PUD_DOWN) ;
	pullUpDnControl (PAUSE, PUD_DOWN) ;
	wiringPiISR (PIN_ENT, INT_EDGE_RISING, flags_ISR);
	wiringPiISR (PLAY, INT_EDGE_RISING, botonStart);
	wiringPiISR (STOP, INT_EDGE_RISING, botonStop);
	wiringPiISR (PAUSE, INT_EDGE_RISING, botonPause);
	return 0;
}
